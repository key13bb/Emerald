...
import star.key13bb.emerald.MyApplication.Companion.getContext
import star.key13bb.emerald.UserSettings
import star.key13bb.emerald.proto.userPreferencesDataStore
...
fun SettingSwitch(
    ...
    LaunchedEffect(key) {
        getContext().userPreferencesDataStore.data.collect { userPreferences ->
            checked = when (key) {
...
suspend fun settingWriter(key: Int, value: Boolean): Boolean {
    when(key) {
        2 -> getContext().userPreferencesDataStore.updateData { preferences -> preferences.toBuilder().setFirebaseAnalytics(value).build() }
        3 -> getContext().userPreferencesDataStore.updateData { preferences -> preferences.toBuilder().setFirebaseCrashlytics(value).build() }
        4 -> getContext().userPreferencesDataStore.updateData { preferences -> preferences.toBuilder().setOnMetered(value).build() }
        else -> throw(Exception(EXCEPTION_KEY))
    }
    return !value
}
